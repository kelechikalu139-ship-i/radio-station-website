...
export default function SchedulePage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      // fetch schedules, programs and oaps in parallel
      ...
      if (progRes.status === "fulfilled") {
        const plist = Array.isArray(progRes.value.data.programs) ? progRes.value.data.programs : progRes.value.data;
        setPrograms(plist || []);
      }
      if (oapRes.status === "fulfilled") {
        const olist = Array.isArray(oapRes.value.data.oaps) ? oapRes.value.data.oaps : oapRes.value.data;
        setOaps(olist || []);
      }
    } catch (err) {
  ...
  function openEdit(s) {
    // Convert DB datetime to local datetime-local input value (YYYY-MM-DDTHH:MM)
  ...
  async function handleSave(e) {
    ...
    if (new Date(form.start_at) >= new Date(form.end_at)) return alert("End must be after start.");
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button onClick={openCreate} className="px-4 py-2 bg-purple-700 text-white rounded">New Schedule</button>
        </div>
      </div>
              ...
              <div key={s.id} className="bg-white rounded shadow p-4 flex gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-sm text-gray-600">
                    {/* <div><strong>When:</strong> {format (new Date(s.start_at), "PP p")} — {format(new Date(s.end_at), "PP p")}</div> */}
                    <div><strong>When:</strong> {(new Date(s.start_at), "PP p")} — {(new Date(s.end_at), "PP p")}</div>
                    {s.notes && <div className="mt-1">{s.notes}</div>}
                  </div>

                  <div className="mt-3 flex gap-2">
                    <button onClick={() => openEdit(s)} className="px-3 py-1 bg-yellow-100 rounded text-sm">Edit</button>
                    <button onClick={() => handleDelete(s.id)} className="px-3 py-1 bg-red-100 rounded text-sm">Delete</button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <form onSubmit={handleSave} className="bg-white w-full max-w-2xl rounded p-6 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select name="program_id" value={form.program_id} onChange={handleChange} className="w-full border rounded p-2" required>
              ...
              <div>
                ...
                <select name="oap_id" value={form.oap_id} onChange={handleChange} className="w-full border rounded p-2" required>
                  <option value="">Select OAP</option>
                  {oaps.map((o) => <option key={o.id} value={o.id}>{o.name}</option>)}
                </select>
              </div>

              <div>
                <label className="text-sm block mb-1">Start (local)</label>
                <input type="datetime-local" name="start_at" value={form.start_at} onChange={handleChange} className="w-full border rounded p-2" required />
              </div>

              <div>
                <label className="text-sm block mb-1">End (local)</label>
                <input type="datetime-local" name="end_at" value={form.end_at} onChange={handleChange} className="w-full border rounded p-2" required />
              </div>

              <div className="md:col-span-2">
                <label className="text-sm block mb-1">Notes (optional)</label>
                <textarea name="notes" value={form.notes} onChange={handleChange} className="w-full border rounded p-2" rows={3}></textarea>
              </div>
            </div>
